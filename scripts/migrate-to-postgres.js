...
async function migrateModel({ log }, config) {
	...
	while (processedCount < totalCount) {
		...
		if (mongoModel === "hub") {
			...
			records = records.filter((/** @type {any} */ record) => {
				const isValid = users.some(
					(/** @type {any} */ u) => u.id === record.ownerId,
				);
				if (!isValid) {
					log(
			...
			records = records.filter((/** @type {any} */ record) => {
				const isValid =
					serverIdSet.has(record.serverId) && hubIdSet.has(record.hubId);
				if (!isValid) {
					log(
						`Skipping connection ${record.id} - Referenced server or hub not found\n`,
					);
				}
				return isValid;
			});
			...
			records = records.filter(
				(
					...
					if (!isValid)
						log(
							`Skipping infraction ${record.id} - Referenced hub or moderator not found\n`,
						);
					return isValid;
				},
			);
			...
			records = records.filter((/** @type {any} */ record) => {
				const isValid =
					hubIdSet.has(record.hubId) && userIdSet.has(record.userId);
				if (!isValid) {
					log(
			...
			records = records.filter((/** @type {any} */ record) => {
				const isValid =
					userIdSet.has(record.userId) && hubIdSet.has(record.hubId);
				if (!isValid) {
					log(
						`Skipping hub upvote ${record.id} - Referenced user (${record.userId}) not found\n`,
					);
				}
				return isValid;
			});
		...
		const transformedRecords = records.map((/** @type {any} */ record) =>
			transform(record),
		);
		...
		try {
			...
			log(
				`Batch complete: ${result.count} ${name.toLowerCase()} migrated successfully`,
			);
		} catch (error) {
			// If batch insert fails, fall back to individual inserts
			log(
				`Batch insert failed, falling back to individual inserts: ${error.message}`,
			);
			...
			for (const record of transformedRecords) {
				try {
					...
					errorCount++;
					log(
						`Error migrating ${name.toLowerCase()} ${getDisplayName(record)}: ${err.message}`,
					);
		...
		log(
			`Progress: ${processedCount}/${totalCount} ${name.toLowerCase()} processed`,
		);
	...
	log(
		`Migrated ${successCount} ${name.toLowerCase()} successfully, ${errorCount} errors`,
	);
	...
	},
	{
		...
		transform: (/** @type {any} */ hub) => {
			...
			if (typeof hub.settings === "number" && hub.settings > 2147483647) {
				console.log(
					`Converting large settings value for hub ${hub.id}: ${hub.settings} -> 0`,
				);
...
async function migrateData() {
	...
	try {
		...
		const logStream = fs.createWriteStream(
			path.join(logDir, `migration-${Date.now()}.log`),
		);
		/**
		...
		for (const config of modelConfigs) {
			try {
				...
				results[config.name.toLowerCase()] = await migrateModel(
					context,
					config,
				);
